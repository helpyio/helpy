class BackupJob < ActiveJob::Base
  ...
  def perform(params, user_id)
    params.select { |k| params[k] == "1" }.each_keys do |model_name|
      klass = Object.const_get model_name
      records = klass.all.order(:id)
      user = User.find(user_id)
      backup = Backup.new(csv: to_csv(records), user_id: user_id, csv_name: "#{model_name}_#{Time.current.to_i}.csv", model: model_name)
      if backup.save
