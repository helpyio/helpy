...
class API::V1::TopicsTest < ActiveSupport::TestCase
  ...
  test "listing all private, open tickets shows all open, private tickets" do
    tickets_to_test_with=10
    tickets_to_close=tickets_to_test_with/2
    # clear the database
    Topic.destroy_all
    to_close = []
    # find the forums to use, one public, one private.
    private_forum = Forum.where(private: true).first
    public_forum = Forum.where(private: false).first

    (1..tickets_to_test_with).each do |x|
      base_values = { user: User.last, name: "Test Ticket #{x}" }
      close_values = base_values.merge({ forum: private_forum, private: true })
      open_values = base_values.merge({ forum: public_forum, private: false })

      to_close.push(Topic.create(close_values))
      Topic.create(open_values)
    end

    # now close half the private tickets.
    (1..tickets_to_close).each do |t|
      to_close[t].close
    end

    get '/api/v1/tickets/private/open', @default_params
    tickets = JSON.parse(last_response.body)
    assert_equal tickets.count, tickets_to_close
