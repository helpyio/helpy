...
class Admin::TopicsControllerTest < ActionController::TestCase
  ...
  %w(admin agent).each do |admin|
    ...
    test "an #{admin} should be able to see a list of topics via ajax" do
      ...
      get :index, params: { status: "open" }, format: :js, xhr: true
      assert_not_nil assigns(:topics)
      assert_template "admin/topics/index"
      assert_response :success
    end
    ...
    test "an #{admin} should be able to see a specific topic of each type via ajax" do
      sign_in users(admin.to_sym)
      [3,7].each do |topic_id|
        get :show, params: { id: topic_id }, xhr: true
        assert_not_nil assigns(:topic)
        assert_template "admin/topics/show"
        assert_response :success
      end
        ...
        assert_difference "Post.count", 1 do
          assert_difference "User.count", 1 do
            assert_difference "ActionMailer::Base.deliveries.size", 2 do
              post :create, params: {
                topic: { user: { name: "a user", email: "anon@test.com" }, name: "some new private topic", post: { body: "this is the body" }, forum_id: 1, current_status: 'new' }
              }, xhr: true
            end
          end
        end
    ...
    test "an #{admin} should be able to create a new private discussion for an existing user with an email" do
      ...
      assert_difference "Topic.count", 1 do
        assert_difference "Post.count", 1 do
          post :create, params: {
            topic: { user: { name: "scott", email: "scott.miller@test.com" }, name: "some new private topic", post: { body: "this is the body" }, forum_id: 1, current_status: 'new' }
          }, xhr: true
        end
      end
    end
    ...
    test "an #{admin} should be able to create a new private discussion for a new user with a phone number" do
      ...
      assert_difference "Topic.count", 1 do
        assert_difference "Post.count", 1 do
          assert_difference "User.count", 1 do
            post :create, params: {
              topic: { user: { name: "a user", home_phone: '34526668', email: "change@me-34526668.com" }, name: "some new private topic", post: { body: "this is the body" }, forum_id: 1, current_status: 'new'}
            }, xhr: true
          end
        end
      end
    ...
    test "an #{admin} created private discussion should have channel" do
      ...
      assert_difference "Topic.count", 1 do
        assert_difference "Post.count", 1 do
          assert_difference "User.count", 1 do
            post :create, params: {
               topic: { user: { name: "a user", work_phone: '34526668', email: "change@me-34526668.com" }, name: "some new private topic", post: { body: "this is the body" }, channel: "phone", forum_id: 1 , current_status: 'new'}
            }, xhr: true
          end
        end
      end
      ...
      assert_difference "Topic.count", 1 do
        assert_difference "Post.count", 1 do
          assert_no_difference "User.count" do
            assert_difference "ActionMailer::Base.deliveries.size", 2 do
              post :create, params: {
                topic: { user: { name: "Scott Smith", email: "scott.smith@test.com" }, name: "some new private topic", post: { body: "this is the body" }, forum_id: 1, current_status: 'new' }
              }, xhr: true
            end
          end
        end
